...
const VideoGeneration: React.FC = () => {
  ...
  // Генерация только промпта (без создания задачи видео)
  const handleGeneratePromptOnly = async (idea: Idea) => {
    if (!selectedChannel) {
      setError('Выберите канал')
      return
    }

    // Устанавливаем состояние загрузки для этой идеи
    setIsPromptLoading(prev => new Map(prev).set(idea.id, true))
    setError('')
    setSuccess('')

    try {
      // Генерируем промпт из идеи
      const promptData = await generatePromptFromIdea(idea, selectedChannel)
      ...
      console.error('[Prompt] Error generating prompt from idea:', err)
      setError(err.message || 'Ошибка при генерации промпта')
      toast.error(err.message || 'Ошибка при генерации промпта')
    } finally {
      setIsPromptLoading(prev => {
        const newMap = new Map(prev)
        newMap.set(idea.id, false)
        return newMap
      })
    }
  }
  ...
  const handleGenerateVideoFromIdea = async (idea: Idea) => {
    ...
    // Устанавливаем состояние загрузки для этой идеи
    setIsVideoLoading(prev => new Map(prev).set(idea.id, true))
    setError('')
    setSuccess('')
                ...
                {ideas.map((idea, index) => (
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '0.5rem' }}>
                      ...
                      {(isPromptLoading.get(idea.id) || isVideoLoading.get(idea.id)) && (
                        <span style={{ color: '#667eea' }}>
                          {isVideoLoading.get(idea.id) ? '⏳ Генерация видео...' : '⏳ Генерация промпта...'}
                        </span>
                      )}
                    </div>
                    ...
                    {selectedIdea?.id !== idea.id && (
                      <div className="idea-actions">
                        ...
                        <button
                          className="button idea-action-button idea-action-button--primary"
                          onClick={(e) => {
                            e.stopPropagation()
                            handleGenerateVideoFromIdea(idea)
                          }}
                          disabled={isVideoLoading.get(idea.id) || isPromptLoading.get(idea.id) || loading || activeJobsCount >= maxActiveJobs}
                        >
                          {isVideoLoading.get(idea.id) 
                            ? '⏳ Генерация видео...' 
                            : '🎬 Сгенерировать видео'}
                        </button>
                        {/* Кнопка "Сгенерировать промпт" */}
                        <button
                          className="button button-secondary idea-action-button idea-action-button--secondary"
                          onClick={(e) => {
                            e.stopPropagation()
                            handleGeneratePromptOnly(idea)
                          }}
                          disabled={isVideoLoading.get(idea.id) || isPromptLoading.get(idea.id) || loading}
                        >
                          {isPromptLoading.get(idea.id) 
                            ? '⏳ Генерация промпта...' 
